...
public class Module {
	private static final double WHEEL_RADIUS = DriveConstants.TrainConstants.kWheelDiameter
			/ 2;
	...
	public Module(ModuleIO io, int index) {
		...
		case REAL:
		case REPLAY:
			driveFeedforward = DriveConstants.TrainConstants.overallDriveMotorConstantContainer
					.getFeedforward();
			driveFeedback = DriveConstants.TrainConstants.overallDriveMotorConstantContainer
					.getPidController();
			turnFeedback = DriveConstants.TrainConstants.overallTurningMotorConstantContainer
					.getPidController();
			break;
		case SIM:
			driveFeedforward = DriveConstants.TrainConstants.overallDriveMotorConstantContainer
	...
	public void periodic() {
		...
		if (turnRelativeOffset == null
			...
			turnRelativeOffset = inputs.turnAbsolutePosition
					.minus(inputs.turnPosition);
		}
		// Run closed loop turn control
		if (angleSetpoint != null) {
			io.setTurnVoltage(turnFeedback.calculate(getAngle().getRadians(),
			...
			if (speedSetpoint != null) {
				...
				io.setDriveVoltage(driveFeedforward.calculate(velocityRadPerSec)
						+ driveFeedback.calculate(inputs.driveVelocityRadPerSec,
								velocityRadPerSec));
			}
		}
		...
		for (int i = 0; i < sampleCount; i++) {
			...
			Rotation2d angle = inputs.odometryTurnPositions[i]
					.plus(turnRelativeOffset != null ? turnRelativeOffset
							: new Rotation2d());
