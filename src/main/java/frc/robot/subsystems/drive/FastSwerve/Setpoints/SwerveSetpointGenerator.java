package frc.robot.subsystems.drive.FastSwerve.Setpoints;

import static frc.robot.utils.drive.EqualsUtil.*;
...
public class SwerveSetpointGenerator {
	 ...
	 * @return True if the shortest path to achieve this rotation involves
	 *         flipping the drive direction.
	 */
	...
	/**
	 * Find the root of the generic 2D parametric function 'func' using the
	 * regula falsi technique. This is a pretty naive way to do root finding, but
	 * it's usually faster than simple bisection while being robust in ways that
	 * e.g. the Newton-Raphson method isn't.
	 *
	 ...
	 */
	private double findRoot(Function2d func, double x_0, double y_0, double f_0,
		...
		if (Math.signum(f_0) == Math.signum(f_guess)) {
			...
			return s_guess + (1.0 - s_guess) * findRoot(func, x_guess, y_guess,
					f_guess, x_1, y_1, f_1, iterations_left - 1);
		} else {
			// Use lower bracket.
			return s_guess * findRoot(func, x_0, y_0, f_0, x_guess, y_guess,
					f_guess, iterations_left - 1);
		}
	}

	protected double findSteeringMaxS(double x_0, double y_0, double f_0,
			double x_1, double y_1, double f_1, double max_deviation,
			int max_iterations) {
	...
	protected double findDriveMaxS(double x_0, double y_0, double f_0,
			double x_1, double y_1, double f_1, double max_vel_step,
			int max_iterations) {
		double diff = f_1 - f_0;
	 ...
	 * @param prevSetpoint The previous setpoint motion. Normally, you'd pass in
	 *                        the previous iteration setpoint instead of the
	 ...
	 */
	public SwerveSetpoint generateSetpoint(final ModuleLimits limits,
		...
		if (GeomExtensions.epsilonEquals(GeomUtil.toTwist2d(desiredState),
				new Twist2d())) {
			need_to_steer = false;
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			if (prevSetpoint.moduleStates()[i].speedMetersPerSecond < 0.0) {
				prev_heading[i] = prev_heading[i]
						.rotateBy(Rotation2d.fromRadians(Math.PI));
			}
			desired_vx[i] = desiredModuleState[i].angle.getCos()
					* desiredModuleState[i].speedMetersPerSecond;
			desired_vy[i] = desiredModuleState[i].angle.getSin()
					* desiredModuleState[i].speedMetersPerSecond;
			desired_heading[i] = desiredModuleState[i].angle;
			if (desiredModuleState[i].speedMetersPerSecond < 0.0) {
				desired_heading[i] = desired_heading[i]
						.rotateBy(Rotation2d.fromRadians(Math.PI));
			}
			if (all_modules_should_flip) {
				double required_rotation_rad = Math.abs(prev_heading[i].unaryMinus()
						.rotateBy(desired_heading[i]).getRadians());
		...
		if (all_modules_should_flip
				&& !(GeomExtensions.epsilonEquals(
						GeomUtil.toTwist2d(prevSetpoint.chassisSpeeds()),
						new Twist2d()))
				&& !(GeomExtensions.epsilonEquals(GeomUtil.toTwist2d(desiredState),
						new Twist2d()))) {
		...
		for (int i = 0; i < modules.length; ++i) {
			if (!need_to_steer) {
				overrideSteering
						.add(Optional.of(prevSetpoint.moduleStates()[i].angle));
				continue;
			}
			overrideSteering.add(Optional.empty());
			if (epsilonEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond,
					0.0)) {
				...
				if (epsilonEquals(desiredModuleState[i].speedMetersPerSecond,
						0.0)) {
					// Goal angle doesn't matter. Just leave module at its current angle.
					overrideSteering.set(i,
							Optional.of(prevSetpoint.moduleStates()[i].angle));
					continue;
				}
				var necessaryRotation = prevSetpoint.moduleStates()[i].angle
						.unaryMinus().rotateBy(desiredModuleState[i].angle);
				if (flipHeading(necessaryRotation)) {
					necessaryRotation = necessaryRotation
							.rotateBy(Rotation2d.fromRadians(Math.PI));
				}
				// getRadians() bounds to +/- Pi.
				final double numStepsNeeded = Math
						.abs(necessaryRotation.getRadians()) / max_theta_step;
				if (numStepsNeeded <= 1.0) {
					// Steer directly to goal angle.
					overrideSteering.set(i,
							Optional.of(desiredModuleState[i].angle));
					...
					overrideSteering.set(i,
							Optional.of(prevSetpoint.moduleStates()[i].angle
									.rotateBy(Rotation2d.fromRadians(
			...
			double s = findSteeringMaxS(prev_vx[i], prev_vy[i],
					prev_heading[i].getRadians(), desired_vx[i], desired_vy[i],
					desired_heading[i].getRadians(), max_theta_step, kMaxIterations);
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			double vx_min_s = min_s == 1.0 ? desired_vx[i]
					: (desired_vx[i] - prev_vx[i]) * min_s + prev_vx[i];
			double vy_min_s = min_s == 1.0 ? desired_vy[i]
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			final var deltaRotation = prevSetpoint.moduleStates()[i].angle
					.unaryMinus().rotateBy(retStates[i].angle);
