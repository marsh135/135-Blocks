...
/**
 * this class stores and displays a competition field that includes the robot,
...
public class CompField {
	...
	public Set<ObjectOnFieldDisplay> clearObjectsWithGivenType(String typeName) {
		if (!objectsOnFieldWithGivenType.containsKey(typeName))
			return new HashSet<>();
		final Set<ObjectOnFieldDisplay> originalSet = objectsOnFieldWithGivenType
				.get(typeName);
	...
	public void updateObjectsToDashboardAndTelemetry() {
		for (String typeName : objectsOnFieldWithGivenType.keySet()) {
			final Set<ObjectOnFieldDisplay> objects = objectsOnFieldWithGivenType
					.get(typeName);
	...
	private static Pose3d[] getPose3ds(Set<ObjectOnFieldDisplay> objects) {
		return objects.stream().map(ObjectOnFieldDisplay::getPose3d)
				.toArray(Pose3d[]::new);
	}
}
