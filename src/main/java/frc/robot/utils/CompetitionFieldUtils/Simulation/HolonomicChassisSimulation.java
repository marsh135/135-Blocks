...
public abstract class HolonomicChassisSimulation extends Body
		implements RobotOnFieldDisplay {
	...
	public HolonomicChassisSimulation(RobotProfile profile,
		...
		super.addFixture(
				Geometry.createRectangle(WIDTH_IN_WORLD_REFERENCE,
						HEIGHT_IN_WORLD_REFERENCE),
				profile.robotMass / (profile.height * profile.width),
				DriveConstants.RobotPhysicsSimulationConfigs.ROBOT_BUMPER_COEFFICIENT_OF_FRICTION,
				DriveConstants.RobotPhysicsSimulationConfigs.ROBOT_BUMPER_COEFFICIENT_OF_RESTITUTION);
	...
	/**
	 * sets the robot's speeds to a given chassis speeds the robot's speeds will
	 * jump to the given speeds in a tick this is different from
	...
	public void simulateChassisBehaviorWithRobotRelativeSpeeds(
		...
		simulateChassisBehaviorWithFieldRelativeSpeeds(ChassisSpeeds
	...
	protected void simulateChassisBehaviorWithFieldRelativeSpeeds(
		...
		final double desiredRotationalMotionPercent = desiredChassisSpeedsFieldRelative.omegaRadiansPerSecond
				/ profile.maxAngularVelocity;
		simulateChassisRotationalBehavior(
				constrainMagnitude(desiredRotationalMotionPercent, 1));
	}

	protected void simulateChassisTranslationalBehavior(
			Vector2 desiredLinearMotionPercent) {
		final boolean robotRequestedToMoveLinearly = desiredLinearMotionPercent
				.getMagnitude() > 0.03;
		...
		final Vector2 forceVec = desiredLinearMotionPercent.copy()
				.multiply(this.profile.propellingForce);
		super.applyForce(new Force(forceVec));
	}

	protected void simulateTranslationalFriction() {
		final double actualLinearPercent = getLinearVelocity().getMagnitude()
				/ profile.robotMaxVelocity;
		final boolean robotActuallyMovingLinearly = actualLinearPercent > 0.03;
		if (robotActuallyMovingLinearly)
			super.applyForce(new Force(super.linearVelocity.getNormalized()
					.multiply(-profile.frictionForce)));
		else
			super.setLinearVelocity(new Vector2());
	}

	private void simulateChassisRotationalBehavior(
			double desiredRotationalMotionPercent) {
		final double maximumTorque = this.profile.maxAngularAcceleration
		...
		final double actualRotationalMotionPercent = Math
				.abs(getAngularVelocity() / profile.maxAngularVelocity),
				frictionalTorqueMagnitude = this.profile.angularFrictionAcceleration
						* super.getMass().getInertia();
		if (actualRotationalMotionPercent > 0.03)
			super.applyTorque(Math.copySign(frictionalTorqueMagnitude,
	 ...
	 */
	public abstract void updateSimulationSubPeriod(int iterationNum,
			double subPeriodSeconds);

	public static final class RobotProfile {
		public final double robotMaxVelocity, robotMaxAcceleration, robotMass,
				propellingForce, frictionForce, linearVelocityDamping,
				maxAngularVelocity, maxAngularAcceleration, angularDamping,
				angularFrictionAcceleration, width, height;

		public RobotProfile(double robotMaxVelocity, double robotMaxAcceleration,
				double maxAngularVelocity, double robotMass, double width,
				double height) {
			this(robotMaxVelocity, robotMaxAcceleration,
					DriveConstants.RobotPhysicsSimulationConfigs.FLOOR_FRICTION_ACCELERATION_METERS_PER_SEC_SQ,
					maxAngularVelocity,
					DriveConstants.RobotPhysicsSimulationConfigs.MAX_ANGULAR_ACCELERATION_RAD_PER_SEC_SQ,
					DriveConstants.RobotPhysicsSimulationConfigs.TIME_CHASSIS_STOPS_ROTATING_NO_POWER_SEC,
					robotMass, width, height);
		}

		public RobotProfile(double robotMaxVelocity, double robotMaxAcceleration,
				double floorFrictionAcceleration, double maxAngularVelocity,
				double maxAngularAcceleration, double timeChassisStopsRotating,
				double robotMass, double width, double height) {
			...
			this.angularFrictionAcceleration = maxAngularVelocity
		...
		public String toString() {
			return String.format(
					"RobotProfile { robotMaxVelocity=%.2f, robotMaxAcceleration=%.2f, robotMass=%.2f, "
							...
							+ "height=%.2f }",
					robotMaxVelocity, robotMaxAcceleration, robotMass,
					propellingForce, frictionForce, linearVelocityDamping,
					maxAngularVelocity, maxAngularAcceleration, angularDamping,
					angularFrictionAcceleration, width, height);
		}
