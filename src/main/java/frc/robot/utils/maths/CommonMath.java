...
public class CommonMath {
	...
	public static double linearInterpretationWithBounding(double x1, double y1,
		...
		return linearInterpretation(x1, y1, x2, y2,
				Math.min(maxX, Math.max(minX, x)));
	}

	public static double linearInterpretation(double x1, double y1, double x2,
			double y2, double x) {
		return y1 + (x - x1) * (y2 - y1) / (x2 - x1);
	}

	/**
	 * using the random number generator of a fixed seed, generate the next
